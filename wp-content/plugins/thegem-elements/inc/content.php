...
function thegem_products_grid($params) {
	...
	echo do_shortcode('[product_category per_page="' . $params['items_per_page'] . '" columns="' . $params['layout_columns'] . '" orderby="' . $params['orderby'] . '" category="' . $params['categories'] . '" order="' . $params['order'] . '" thegem_grid_params="' . htmlspecialchars(serialize($params)) . '"]');
}
...
function thegem_video_background($video_type, $video, $aspect_ratio = '16:9', $headerUp = false, $color = '', $opacity = '', $poster='') {
	...
	if($video_type && $video) {
		...
		if($video_type == 'youtube' || $video_type == 'vimeo') {
			...
			$video_block = '<iframe class="gem-video-background-iframe" src="'.esc_url($link).'" frameborder="0"></iframe>';
		} else {
...
function thegem_product_slider($params) {
	...
	echo do_shortcode('[product_category per_page="-1" columns="4" category="' . $params['categories'] . '" thegem_slider_params="' . htmlspecialchars(serialize($params)) . '"]');
}
