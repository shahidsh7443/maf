...
function thegem_replace_ids_attachments_content($data, $attachment_ids) {
    ...
    if(is_numeric($match[0])) {
        $match_id = $match[0];
        $str=str_replace($match_id,$attachment_ids[$match_id],$data);
    } else {
        $match_ids = explode(',',$match[0]);
        $copy_item = $data;
        foreach ($match_ids as $match_id) {
            if($attachment_ids[$match_id]) {
                $copy_item=str_replace($match_id,$attachment_ids[$match_id],$copy_item);
            } else {
...
function sort_array_for_single_item($array) {
    ...
    for ($i = 0; $i < count($array); $i++) {
        if(!empty($array[$i]['children'])) {
            uasort($array[$i]['children'], function($a, $b) {
            ...
            for($x = 0; $x < count($array[$i]['children']); $x++) {
                if(!empty($array[$i]['children'][$x]['children'])) {
                    uasort($array[$i]['children'][$x]['children'], function($a2, $b2) {
                        if(!empty($a2['parent']) && !empty($b2['parent']))  {
                            return strcmp($a2['parent'], $b2['parent']);
                        }
                        if(!empty($a['title']) && !empty($b['title']))  {
                            return strcmp($a2['title'], $b2['title']);
                        }
                        return 0;
                    });
                }
